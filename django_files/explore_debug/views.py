...
from explore_debug.models import *

def home(request):
   return render_to_response("debug/home.html")
...
def explore_profile(entity_id, plot_type, target_id):
   ...
   return render_to_response("debug/profile.html", {
      'id': entity.id,
...
def explore_network(network_id):
   if network_id == "1":
      ...
      type = "IS"
   return render_to_response("debug/network.html", {
      'type': type,
      'section': "network",
   })
...
def explore_ranking(ranking_id):
   ...
   return render_to_response("debug/ranking.html", {
      'type': type,
      'rankings': rankings,
      'section': "ranking",
   })
...
def get_data_bipartite(entity_id, target_id):
   ...
   for point in points_y:
      if point.entity_trg.type_of_entity != entity.type_of_entity and point.entity_trg.name in temp_records:
         ...
         if record["x"] > max_x:
            ...
            min_x = record["x"]
         record["y"] = point.rca_adj
         if point.rca_adj > max_y:
            max_y = point.rca_adj
         elif point.rca_adj < min_y:
            min_y = point.rca_adj
   ...
   response_data["trendlabel"] = u"%s<br />%s<hr />\u03B1 = %1.4f" % (entity.name.title(), target.name.title(), m)
...
def get_data_network(network_id, entity_type):
   ...
   response_data["links"] = []
   edges = Entity.objects.filter(type_of_entity = entity_type)
   for edge in edges:
      record = {}
...
def about(request, about_type):
   if about_type == "self":
      return render_to_response("debug/about.html", {
         "section": "about",
      })
   elif about_type == "data":
      orgs = Entity.objects.filter(type_of_entity = "OR")
      return render_to_response("debug/about_data.html", {
         "entities": orgs,
         "head": "Organization",
         "type": "data",
         "section": "about",
      ...
      return render_to_response("debug/about_team.html", {
         "section": "about",
      })
   elif about_type == "glossary":
      return render_to_response("debug/about_glossary.html", {
         "section": "about",
      })
